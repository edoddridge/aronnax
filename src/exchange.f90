module exchange
  ...
  subroutine collect_global_array(array, global_array, nx, ny, layers, ilower, iupper, &
                          xlow, xhigh, ylow, yhigh, OL, num_procs, myid)
    implicit none

    double precision, intent(in)  :: array(xlow-OL:xhigh+OL, &
                                              ylow-OL:yhigh+OL, layers)
    double precision, intent(out) :: global_array(1-OL:nx+OL, 1-OL:ny+OL, layers)
    integer,          intent(in)  :: nx, ny, layers
    integer,          intent(in)  :: ilower(0:num_procs-1,2)
    integer,          intent(in)  :: iupper(0:num_procs-1,2)
    integer,          intent(in)  :: xlow, xhigh, ylow, yhigh, OL
    ...
    return
  end subroutine collect_global_array
